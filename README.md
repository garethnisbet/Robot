...
`check_pose`, `check_trajectory`, `plan_path` and `execute_path` (MCP) and `robot.plan()` take their verdict from this engine, run on a copy of the viewer's scene (`exportScene`, via `headless_client.py`). Verdicts name their source in `checked_by`. Without Node, or with a viewer page older than `exportScene`, they fall back to the planner's capsules and say so.

The planner searches with capsules fitted to enclose each link's mesh (`<name>_capsules.json`, generated by `node --import ./headless/register.mjs headless/fit-capsules.mjs`). Because the capsules enclose the meshes, the planner may refuse a pose the viewer would allow, but it never passes one the viewer would reject. `test/js/capsules.test.mjs` checks the enclosure for every config, so regenerate the capsules after changing a model. The capsules are not used for self-collision: links two apart nest into each other at a compact wrist, so enclosing shapes overlap in almost every pose. Self-collision is left to the exact check on the finished path.

The planner takes the scene from the viewer (`RobotPlanner.sync_from_viewer`):
- **Where the device stands.** Its capsules are placed at its world pose.
- **Other visible devices.** Each becomes an obstacle made of its own fitted capsules, at its current joint angles. It uses each link's `parts`: several capsules whose union encloses the link, so they hug irregular shapes such as a diffractometer circle far closer than one capsule does. The moving arm keeps one capsule per link; parts there made no measurable difference to which goals were reachable, and slowed planning.
...
headless_client.py       Runs the headless engine from Python (MCP server, robot.plan)
*_capsules.json          Per-link enclosing capsules for the planner (generated)
