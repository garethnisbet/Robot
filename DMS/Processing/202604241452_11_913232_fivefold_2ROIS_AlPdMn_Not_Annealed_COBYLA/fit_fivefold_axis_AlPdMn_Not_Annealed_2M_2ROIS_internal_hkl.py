...
#     minimizer_kwargs = {"method": "COBYLA"}
    if OptMethod == 'GA':
        ...
        print('Using ' + OptMethod + ' with ' + str(_n_cores) + '-start parallel search')
        _rng = np.random.default_rng(42)
        _starts = [ig] + [ig + _rng.uniform(-0.5, 0.5, ig.shape) for _ in range(_n_cores - 1)]
        ...
        _results = Parallel(n_jobs=-1)( delayed(_run_one)(s) for s in _starts )
