...
width=25*zoomval # roi width
comwidth = 5*zoomval

scan=scannum=708458 # fivefold
scan=scannum=913111 # AlPdMn December 2021 Annealed with phason strain
...
scan=scannum=913123 # AlPdMn Annealed
# scan=scannum=913232 # AlPdMn December 2021 Not Annealed No phason strain


firstplot = 1 # set to 1 if you want to create a new plot. Set to 0 if you want to overl the new plots
datapoint0=8
datapoint=3
...
bravais = 'icosahedral'
# bravais = 'calibrate'
# bravais = 'icosahedral_fixed_a'
# bravais = 'cubic_no_strain'

autoreflist=0 # Only set to one if you have a cif file
...
cif_file='/home/ndf61257/Python/calcms/cif/PMN-PT_Pm-3m_161999.cif' # 1
# scanpath ='/home/ndf61257/MintSpace/i16extra/data/2018/mt18876-1/'

# scanpath ='/home/ndf61257/MintSpace/i16extra/data/2018/mt18876-1/'
scanpath ='/home/ndf61257/MintSpace/i16extra/data/2021/mm29043-1/'
# scanpath ='/Barracuda/Python/Quasicrystal/Data/2021/mm29043-1/'
...
hkl = np.array([1.90672, 4.00566 ,1.39568])
hkl = np.array([1.90755558, 4.00651662, 1.39816154])

#hkl=np.array([2.31420248, 3.76877093, 1.44792992])

# hkl = np.array([2.3149776737984498, 3.763732171705426, 1.4372710042635664])
...
#im = misc.imread(str(scanpath + str(imtemplate % imnum)))
...
px=(1004)*zoomval# datapoint 0
py=(938-259)*zoomval # datapoint 6

px=(1018+127)*zoomval# datapoint 0
py=(796+21)*zoomval # datapoint 6

thb=ts.bragg(lattice,hkl,energy).th()[0]
thrange=[thb-27,thb+10] # theta raange for dms calculation
# thrange=[thb-5,thb+5] # theta raange for dms calculation
# thrange=[thb-8,thb+5]
...
initial_guess = np.array([6.461485,6.461485,6.461485,90.000000,90.000000,90.000000,0.023283,0.002190,-0.000118,0.002480,9628.715054/2*zoomval,0.615757,0.686783,-1.440771,energy+0.00004667,0.001969,0.001804,0.000771,0.001408,-0.002829,-0.003810,0.000447,-0.000306,0.003065,])
...
#------------------------------------------------------------------------------ 
# centres[18]=11.86*zoomval
centres[20]=26.1/2*zoomval
# centres[21]=12.32*zoomval
# centres[22]=12.26*zoomval
centres[23]=24.1/2*zoomval
centres[24]=26.3/2*zoomval
centres[25]=23.5/2*zoomval
# centres[26]=12.05*zoomval
centres[27]=19.5/2*zoomval
# centres[30]=21.38
# centres[31]=20.52
# centres[35]=18.3
# centres[39]=15.66
# centres[27]=27.1/2*zoomval
# centres[15]=18.25
#                            0_a        1_b    2_c    3_alpha     4_beta  5_gamma  6_psicor  7_hcor   8_kcor   9_lcor   10_detdist              11_dxrot   12_dyrot    13_dzrot    14_energy           15_pmatrix ->
...
#     minimizer_kwargs = {"method": "COBYLA"}
    if OptMethod == 'GA':
        ...
        res = differential_evolution(dms.fit,bounds,strategy=strat,polish=True)
